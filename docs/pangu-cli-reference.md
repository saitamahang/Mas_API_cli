...
> 这是底层 `pangu` CLI 的命令参考。项目入口和 `pangu-agent` 说明见 [README](../README.md)，agent 工作流用法见 [pangu-agent 使用指南](pangu-agent-usage.md)。

盘古平台底层管理 CLI，支持工作空间、模型资产、推理服务、训练任务、数据集等全生命周期管理。
...
- 自动发布：`auto_publish_config{is_auto_publish}`（默认 `true`，训练完成后自动发布为模型版本）
...
# 关闭自动发布
pangu training create -f train.yaml --no-auto-publish
